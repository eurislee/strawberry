:man source:   Strawberry
:man version:  {revnumber}
:man manual:   Strawberry Manual
...
strawberry - Binary space partitioning window manager

Synopsis
--------

*strawberry* [*-h*|*-v*|*-c* 'CONFIG_PATH']

*strawberryc* 'DOMAIN' ['SELECTOR'] 'COMMANDS'

*strawberryc* 'COMMAND' ['OPTIONS'] ['ARGUMENTS']

Description
-----------

*strawberry* is a tiling window manager that represents windows as the leaves of a full binary tree.

It is controlled and configured via *strawberryc*.
...
*-c*, *--count* 'COUNT'::
	Stop the corresponding *strawberryc* process after having received 'COUNT' events.
...
If the server can't handle a message, *strawberryc* will return with a non-zero exit code.
...
Please note that *strawberry* initializes monitors before it reads messages on its socket, therefore the initial monitor events can't be received.
...
'STRAWBERRY_SOCKET'::
	The path of the socket used for the communication between *strawberryc* and *strawberry*. If it isn't defined, then the following path is used: '/tmp/strawberry<host_name>_<display_number>_<screen_number>-socket'.
